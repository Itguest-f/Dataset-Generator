...
class CaseGenerator:
    ...
    def __init__(self, job_init,job_new, num_mas, opes_per_job_min, opes_per_job_max, nums_ope=None, path='./1305/',
        ...
        self.flag_job_new = flag_job_new # new jobs arrive
        self.job_new = job_new  # new jobs number
        ...
        self.proctime_dev = 0.2   # The standard deviation ratio of processing time
    ...
    def get_case(self, idx=0):
        ...
        if self.flag_job_new:
            self.num_jobs = self.job_init + self.job_new # new jobs insertion
        ...
        self.num_ope_biass = [sum(self.nums_ope[0:i]) for i in range(self.num_jobs)] # number of initial processes
        self.num_ma_biass = [sum(self.nums_option[0:i]) for i in range(self.num_opes)] # number of initial machines
        ...
        for i in range(self.num_jobs):
            flag = 0
            flag_time = 0
            flag_new_ope = 1 # flag for the number of optional machines for the operation
            idx_ope = -1
            idx_ma = 0
            line = []
            ...
            while True:
                if flag == 0:
                    line.append(self.nums_ope[i]) # the number of operations for job i
                    flag += 1
                elif flag == flag_new_ope:
                    idx_ope += 1
                    idx_ma = 0
                    flag_new_ope += self.nums_option[self.num_ope_biass[i]+idx_ope] * 2 + 1
                    line.append(self.nums_option[self.num_ope_biass[i]+idx_ope]) # number of optional machines for the operation
                    flag += 1
                elif flag_time == 0:
                    line.append(self.ope_ma[self.num_ma_biass[self.num_ope_biass[i]+idx_ope] + idx_ma]) # machine number
                    flag += 1
                    flag_time = 1
                else:
